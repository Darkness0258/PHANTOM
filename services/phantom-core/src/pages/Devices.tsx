  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
        ...
        <div style={{ fontSize:'11px', color:'#4a6278' }}>
          {devices.length} device{devices.length !== 1 ? 's' : ''} · updates every 5s
        </div>
      </div>

      {loading && (
        <div style={{ color:'#4a6278', fontSize:'13px' }}>Connecting to phantom-core...</div>
      )}

      {!loading && devices.length === 0 && (
        <div style={{ background:'#0b1019', border:'1px solid #141e2e', borderRadius:'11px', padding:'24px', textAlign:'center' }}>
          <div style={{ fontSize:'24px', marginBottom:'8px' }}>📡</div>
          <div style={{ color:'#4a6278', fontSize:'13px' }}>
            No devices reporting yet. Make sure phantom-agent is running.
          </div>
        </div>
      )}
            ...
            <div key={d.id} style={{ background:'#0b1019', border:'1px solid #141e2e', borderRadius:'9px', padding:'13px 15px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'12px' }}>
                ...
                <div style={{ minWidth:0 }}>
                  <div style={{ fontWeight:600, fontSize:'13px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.name}</div>
              ...
              <div style={{ marginBottom:'7px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:'10.5px', color:'#4a6278', marginBottom:'3px' }}>
                  <span>CPU</span>
                  <span style={{ color:cpuColor }}>{d.cpu_usage.toFixed(1)}%</span>
                </div>
              ...
              <div style={{ marginBottom:'10px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:'10.5px', color:'#4a6278', marginBottom:'3px' }}>
                  <span>RAM</span>
                  <span style={{ color:ramColor }}>{d.ram_percent}% · {fmt(d.memory_used)} / {fmt(d.memory_total)}</span>
                </div>
                <div style={{ height:'4px', background:'#141e2e', borderRadius:'2px' }}>
              ...
              <div style={{ fontSize:'10px', color:'#3a4e63' }}>
